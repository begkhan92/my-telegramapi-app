...
using bn_Food_Ordering_Bot.Data;
using bn_Food_Ordering_Bot.Models;
using Microsoft.EntityFrameworkCore;
using bn_Food_Ordering_Bot.Services;

namespace bn_Food_Ordering_Bot.Controllers;
...
public class ManagerApiController : ControllerBase
{
    ...
    [FromServices] VenueHostedService venueHosted)
    {
        ...
        {
            "revenue" => $"💰 *Прибыль — {venue?.Name}*\n\n📅 Сегодня: *{revenueToday:0.00}* ({allOrders.Count(o => o.CreatedAt >= today)} заказов)\n📊 Всего: *{revenueTotal:0.00}* ({allOrders.Count} заказов)\n🧾 Средний чек: *{avgCheck:0.00}*",
    ...
    private string BuildItemsReport(List<Order> confirmed, string venueName)
    {
        ...
        var lines = string.Join("\n", items.Select(i => $"• {i.name} ×{i.qty} — {i.sum:0.00}"));
